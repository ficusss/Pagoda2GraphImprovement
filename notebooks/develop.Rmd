...
qplot(edge_differents)

ToPoint <- function(vec, bins = 25) {
  step <- max(edge_differents) - min(edge_differents)
  z <- seq(min(edge_differents), max(edge_differents), step)
  for (curr in seq(min(edge_differents), max(edge_differents), step)) {
  }
