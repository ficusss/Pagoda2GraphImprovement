...
update_knn_graph <- UpdateNNGraph(pagoda_data$graphs$PCA, clusters_pagoda_data,
                                   all_clusters, 15, graph.type = 'knn')
...
count_edge_change <- CountEdgeChanges(pagoda_data$graphs$PCA, updated_knn_graph)

```
```{r}

modularity_info <- GetModularityForGraphs(list(pagoda_data$graphs$PCA, updated_knn_graph,
                              updated_mnn_graph), c("infomap", "multilevel"))

```

```{r}

EdgeСhangesForGraphs(pagoda_data$graphs$PCA, updated_knn_graph)
EdgeСhangesForGraphs(pagoda_data$graphs$PCA, updated_mnn_graph)
EdgeСhangesForGraphs(pagoda_data$graphs$PCA, pagoda_data$graphs$PCA)

```

```{r}

edge_differents <- Map(f = function(a) a[2] - a[1], count_edge_change)
edge_differents <- Reduce(union, edge_differents)

qplot(edge_differents, bins = 100)
...
plot(g)
# %du% igraph::make_full_graph(5) %du% igraph::make_full_graph(5)
g <- igraph::add_edges(g, c(1,6, 1,11, 6, 11))
plot(g)
wtc <- igraph::cluster_walktrap(g)
igraph::modularity(wtc)
igraph::modularity(g, membership(wtc))
