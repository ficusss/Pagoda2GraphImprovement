...
update_pagoda_data_multilevel <- UpdatePagoda(primary.data = load_matrix, pagoda.obj = pagoda_data,
                                   clustering.type='multilevel', k=30)
...
clusters <- update_pagoda_data$clusters$PCA$infomap
clusters_name <- lapply(levels(clusters), ClusterNeighborhood, clusters, update_pagoda_data)

```
```{r}
# improvement clusters

cluster_graph <- GetPagoda(load_matrix[,clusters_name[[3]]$expand_cluster], k=25, n.pcs = 36)
...
conos::embeddingPlot(cluster_graph$embeddings$PCA$tSNE,
                     groups = update_pagoda_data$clusters$PCA$infomap)
conos::embeddingPlot(cluster_graph$embeddings$PCA$tSNE)
